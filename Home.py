...
st.markdown('''- **Define target variables.** You can define two different target variables for a “toy” hiring algorithm—two different ways of understanding what counts as a successful employee—by assigning different levels of cognitive characteristics (like reasoning skills, and information processing speed) that employees can have.''')      
st.markdown(''''Once you have defined the two target variables, our dashboard will, drawing on a data-set of the cognitive tests from real-world people, build two hiring models, A and B. Model A predicts which candidates will be successful employees on your first definition; model B predicts which candidates will be successful employees on your second definition.''')
